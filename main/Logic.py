...
for i, row in student_data.iterrows():
    ...
    targets.extend([float(x) for x in row[81:]])  # 使用extend将目标变量扩展为一维数组


features = np.array(features, dtype=float)
targets = np.array(targets, dtype=int)  # 将目标变量转换为整数类型
...
result_df.to_csv('D:/研一下/dataAnalyse/sourceFile/Result/scores/logistic_regression_scores.csv', index=False)
